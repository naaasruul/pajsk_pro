<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Co-Curriculum Activity') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('cocuriculum.update', $cocuriculum) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select id="student_id" name="student_id" required
                                    ...
                                    @foreach(\App\Models\Student::with('user')->get() as $student)
                                        <option value="{{ $student->id }}" {{ old('student_id', $cocuriculum->student_id) == $student->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <x-text-input id="no_maktab" name="no_maktab" type="text" class="mt-1 block w-full"
                                    :value="old('no_maktab', $cocuriculum->no_maktab)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="class" name="class" type="text" class="mt-1 block w-full"
                                    :value="old('class', $cocuriculum->class)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="activity" name="activity" type="text" class="mt-1 block w-full"
                                    :value="old('activity', $cocuriculum->activity)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="marks" name="marks" type="number" class="mt-1 block w-full"
                                    :value="old('marks', $cocuriculum->marks)" required min="0" max="100" />
                                <x-input-error :messages="$errors->get('marks')" class="mt-2" />
                            </div>
                        </div>

                        <div class="mt-6 flex items-center gap-4">
                            <x-primary-button>{{ __('Update Activity') }}</x-primary-button>
