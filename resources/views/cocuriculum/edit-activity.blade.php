    ...
    <x-container>
        ...
        <form id="activity-form" method="POST" action="{{ route('activity.update', $activity) }}">
            ...
            <div class="step-content" data-step="1">
                ...
                <div class="mt-6">
                    ...
                    <select id="represent" name="represent"
                        ...
                        @if ($activity->status == 'approved') disabled @endif>
                        <option value="Menghadiri" {{ $activity->represent == 'Menghadiri' ? 'selected' : '' }}>
                            Menghadiri</option>
                        <option value="Menerima Anugerah Cemerlang"
                            {{ $activity->represent == 'Menerima Anugerah Cemerlang' ? 'selected' : '' }}>Menerima
                            Anugerah Cemerlang</option>
                        <option value="Menyertai" {{ $activity->represent == 'Menyertai' ? 'selected' : '' }}>Menyertai
                        </option>
                        <option value="Mewakili Daerah dalam"
                            {{ $activity->represent == 'Mewakili Daerah dalam' ? 'selected' : '' }}>Mewakili Daerah
                            dalam</option>
                    </select>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <select id="involvement" name="involvement_id"
                        ...
                        @foreach ($involvementTypes as $involvementType)
                            <option value="{{ $involvementType->id }}"
                                {{ $activity->involvement_id == $involvementType->id ? 'selected' : '' }}>
                                {{ $involvementType->description }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex ">
                    <div class="mt-6 w-1/2">
                        ...
                        <div class="relative max-w-sm">
                            ...
                            <input id="date-start" value="{{ $activity->date_start }}" datepicker
                                datepicker-format="yyyy-mm-dd" type="text"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Select date" @if ($activity->status == 'approved') disabled @endif>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 w-1/2">
                        ...
                        <div class="relative max-w-sm">
                            ...
                            <input type="time" id="time-start" value="{{ $activity->time_start }}"
                ...
                <div class="flex ">
                    <div class="mt-6 w-1/2">
                        ...
                        <div class="relative max-w-sm">
                            ...
                            <input id="date-end" value="{{ $activity->date_end }}" datepicker
                                datepicker-format="yyyy-mm-dd" type="text"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Select date" @if ($activity->status == 'approved') disabled @endif>
                        </div>
                    ...
                    <div class="mt-6 w-1/2">
                        ...
                        <div class="relative max-w-sm">
                            ...
                            <input type="time" id="time-end" value="{{ $activity->time_end }}"
                                class="bg-gray-50 border leading-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                min="09:00" max="18:00" value="00:00" required
                                @if ($activity->status == 'approved') disabled @endif />
                        </div>
                    ...
                    <select id="club" name="club_id"
                        ...
                        @foreach ($clubs as $club)
                            <option value="{{ $club->id }}"
                                {{ $activity->club_id == $club->id ? 'selected' : '' }}>{{ $club->club_name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select id="achievement" name="achievement_id"
                        ...
                        @foreach ($achievementTypes as $achievementType)
                            <option value="{{ $achievementType->id }}"
                                {{ $activity->achievement_id == $achievementType->id ? 'selected' : '' }}>
                                {{ $achievementType->achievement_name }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select id="placement" name="placement_id"
                        ...
                        @foreach ($placements as $placement)
                            <option value="{{ $placement->id }}"
                                {{ $activity->placement_id == $placement->id ? 'selected' : '' }}>
                                {{ $placement->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mt-6">
                    ...
                    <textarea id="address" rows="4"
                        ...
                        placeholder="Activity address here..." @if ($activity->status == 'approved') disabled @endif>{{ $activity->activity_place }}</textarea>
                </div>
                                    ...
                                    <td class="w-4 p-4">
                                        <div class="flex items-center">
                                            <input id="checkbox-table-search-{{ $teacher->id }}" type="checkbox"
                                                {{ $activity->teachers->contains($teacher->id) ? 'checked' : '' }}
                                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                                    ...
                                    <td class="w-4 p-4">
                                        <div class="flex items-center">
                                            <input id="checkbox-student-{{ $student->id }}" type="checkbox"
                                                {{ $activity->students->contains($student->id) ? 'checked' : '' }}
                                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
            ...
            <div class="mt-6 flex justify-between">
                ...
                @else
                    {{-- Only show placement field, no navigation or submit --}}
                    <button type="submit" id="placement-submit"
